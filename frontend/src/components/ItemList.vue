<template>
  <section>
    ...
    <ul v-else>
      <li v-for="item in items" :key="item.id" @click="share(item)" :class="{ shared: selectedId === item.id }">
        {{ item.title }}
      </li>
    </ul>
  </section>
...
/**
 * Componente que lista items obtenidos desde la API /api/items.
 * Incluye control de carga y errores y sincronización realtime entre ventanas.
 */
export default {
  name: 'ItemList',
  setup() {
    /**
     * Lista de items obtenida desde la API.
    ...
    function handleIncomingMessage(raw) {
      ...
      if (payload && payload.action === 'share_item' && payload.item) {
        ...
        // Opcional: podríamos mostrar una notificación visual; por simplicidad, se aplica clase .shared
    ...
    return { items, loading, error, reload, share, selectedId };
  }
};
</script>
...
.shared { background: rgba(200,230,255,0.6); cursor: pointer; }
li { cursor: pointer; padding: 0.25rem 0.25rem; }
</style>
